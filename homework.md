...
2. (Scheinerman 17.14 modified) You have a set $A$ with $n$-many elements. How many $k$-element subsets $B\subset A$ are there? Explain your answer. How many subsets $B\subset A$ are there in total? Explain your answer. Use the above reasoning to give a proof of the equation: $\sum\_{k=0}^n\binom{n}{k}=2^n$.
3. (Scheinerman 17.16 modified) Recall that $\binom{n}{k}=\frac{n!}{k!(n-k)!}$. Use this formula to prove $k\binom{n}{k}=n\binom{n-1}{k-1}$.
...
5. In our data about $\left(\binom{n}{k}\right)$ we saw that $\left(\binom{n}{k}\right)=\left(\binom{n-1}{k}\right)+\left(\binom{n}{k-1}\right)$. Use ball/urn reasoning to explain why this is the case. Hint: Given a configuration (A) of $n$ labelled urns with $k$ balls in them, if the last urn is empty then delete the urn to obtain a configuration (B), and if the last urn is not empty then delete one of the balls to obtain a new configuration (C). Argue that there is a bijection between configurations of type (A) and the union of the configurations of type (B) and (C).
